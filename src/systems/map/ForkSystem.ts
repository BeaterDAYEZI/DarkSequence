...
export function settleFork(run: RunState, zone: ZoneDef, forkNodeId: string, rail: ForkRail, rng: Rng): ForkResult {
  ...
  // 遗物牌池（牌库+手牌+弃牌堆）
  const deck = run.deck;
  const allPiles = [...deck.drawPile, ...deck.hand, ...deck.discardPile];
  const relicCards = allPiles.filter((c) => !registry.cards.get(c)?.heroId);
  ...
  if (rail === 'memory') {
    ...
    if (relicCards.length === 0) {
      ...
      const target = rng.pick(relicCards);
      // 从各堆中移除一张
      ...
      logs.push({ text: `记忆之轨吞噬了一张遗物牌【${name}】`, kind: 'info' });
    }
    ...
    if (relicCards.length === 0) {
      ...
      const name = registry.cards.get(target)?.name ?? target;
      logs.push({ text: `遗忘之轨复制了一张遗物牌【${name}】`, kind: 'info' });
    }
